/*
 * itb3.h — public C header for the ITB C binding.
 *
 ...
 * (`-litb3_c -litb3`); every hash-name / MAC-name / cipher-name /
 ...
 * underlying libitb3 library version. */
 ...
 * wrapped here but relayed verbatim if libitb3 ever returns them). */
 ...
 * half-supplied pair is rejected by libitb3). The profile shape
 ...
 * libitb3 (a missing or unreadable file is ITB_STATUS_BAD_INPUT with
...
/* Profile records                                                     */
/* ------------------------------------------------------------------ */
/* A profile record is the JSON object libitb3 accepts in itb_register,
 * returns from itb_lookup / itb_inspect, and embeds in every blob.
 * Optional keys are omitted when empty / zero. The binding treats the
 * record as an opaque string; every field rule is enforced by
 * libitb3. */
...
/* The libitb3 library version string (e.g. "0.5.1"). Thread-local
