/* encrypt_stream_one_shot throughput vs plaintext size (Streaming AEAD
 * profile) at 1 MiB / 16 MiB / 64 MiB. Times the whole-buffer path
 * (a single FFI round trip through the Pipeline's stream chain). */
...
static int run_stream_one_shot(void *raw)
{
    struct stream_ctx *ctx = raw;
    uint8_t *wire = NULL;
    size_t wire_len = 0;
    itb_status st = itb_pipeline_encrypt_stream_one_shot(ctx->pipe, ctx->plain,
                                                         ctx->size, &wire,
                                                         &wire_len);
    if (st != ITB_STATUS_OK) {
        return 1;
...
static int run_stream_one_shot_decrypt(void *raw)
{
    struct stream_dec_ctx *ctx = raw;
    uint8_t *plain = NULL;
    size_t plain_len = 0;
    itb_status st = itb_pipeline_decrypt_stream_one_shot(ctx->pipe, ctx->wire,
                                                         ctx->wire_len, &plain,
                                                         &plain_len);
    if (st != ITB_STATUS_OK) {
        return 1;
...
int main(void)
{
    ...
    if (opts == NULL) {
        fprintf(stderr, "bench_stream_one_shot: opts alloc failed\n");
        return 1;
    }
    ...
    itb_status st = itb_pipeline_init(bench_profile_name("streaming-noaead-triple-v1"),
                                      opts, &pipe);
    if (st != ITB_STATUS_OK) {
        fprintf(stderr, "bench_stream_one_shot: init failed: %s\n",
                itb_last_error());
        itb_opts_free(opts);
        return 1;
    }
    ...
    for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++) {
        ...
        if (plain == NULL) {
            fprintf(stderr, "bench_stream_one_shot: out of memory\n");
            return 1;
        }
        ...
        for (size_t o = 0; o < sizes[i];) {
            ...
            if (r <= 0) {
                fprintf(stderr, "bench_stream_one_shot: getrandom failed\n");
                return 1;
            }
            o += (size_t)r;
        }
        struct stream_ctx ctx = { pipe, plain, sizes[i] };
        bench_case("stream_one_shot", sizes[i], run_stream_one_shot, &ctx);
        /* Pre-encrypt one wire for the decrypt timing loop. */
        uint8_t *dec_wire = NULL;
        size_t dec_wire_len = 0;
        if (itb_pipeline_encrypt_stream_one_shot(pipe, plain, sizes[i], &dec_wire,
                                                 &dec_wire_len) != ITB_STATUS_OK) {
            fprintf(stderr, "bench_stream_one_shot: dec setup encrypt failed: %s\n",
                    itb_last_error());
            return 1;
        }
        struct stream_dec_ctx dctx = { pipe, dec_wire, dec_wire_len };
        bench_case("stream_one_shot-dec", sizes[i], run_stream_one_shot_decrypt, &dctx);
