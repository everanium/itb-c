/*
 * bench_triple_stream.c -- Triple Ouroboros streaming benchmarks for
 * the C binding.
 *
 ...
 * ITB key + hmac-blake3 MAC. Same matrix shape as bench_single_stream.c
 * but routed through the 7-seed Triple ABI:
 *
 *   AEAD IO  -- itb_encryptor_stream_encrypt_auth (Easy, mode=3) /
 *               itb_stream_encrypt_auth_triple (Low-Level free fn).
 *   UserLoop -- itb_encryptor_encrypt (Easy, mode=3) /
 *               itb_encrypt_triple (Low-Level free fn).
 *
 * Setup discipline: 64 MiB CSPRNG fill, encryptor / 7 Seeds / MAC
...
/* Triple Ouroboros encryptor: mode=3. The Encryptor abstraction
 * absorbs the seed allocation; the per-encryptor cipher path takes
 * the plaintext directly without explicit seed parameters. */
static itb_encryptor_t *build_stream_encryptor_triple(void) {
    itb_encryptor_t *e = NULL;
    itb_status_t s = itb_encryptor_new(STREAM_PRIMITIVE, STREAM_KEY_BITS,
                                        STREAM_MAC_NAME, 3, &e);
    if (s != ITB_OK || e == NULL) {
        fprintf(stderr, "itb_encryptor_new(triple) failed: %s\n", itb_last_error());
        abort();
    }
    if (env_lock_seed()) {
...
/* Triple Ouroboros 7-seed kit: 1 noise + 3 data + 3 start. */
typedef struct seed_kit_triple {
    itb_seed_t *noise;
    itb_seed_t *d1;
    itb_seed_t *d2;
    itb_seed_t *d3;
...
static void build_stream_seeds_triple(seed_kit_triple_t *kit) {
    if (itb_seed_new(STREAM_PRIMITIVE, STREAM_KEY_BITS, &kit->noise) != ITB_OK ||
        itb_seed_new(STREAM_PRIMITIVE, STREAM_KEY_BITS, &kit->d1) != ITB_OK ||
        itb_seed_new(STREAM_PRIMITIVE, STREAM_KEY_BITS, &kit->d2) != ITB_OK ||
        itb_seed_new(STREAM_PRIMITIVE, STREAM_KEY_BITS, &kit->d3) != ITB_OK ||
        itb_seed_new(STREAM_PRIMITIVE, STREAM_KEY_BITS, &kit->s1) != ITB_OK ||
        itb_seed_new(STREAM_PRIMITIVE, STREAM_KEY_BITS, &kit->s2) != ITB_OK ||
        itb_seed_new(STREAM_PRIMITIVE, STREAM_KEY_BITS, &kit->s3) != ITB_OK) {
        fprintf(stderr, "itb_seed_new (triple) failed: %s\n", itb_last_error());
        abort();
    }
}
...
/* ---- Per-case context type ---------------------------------------- */

typedef struct case_ctx {
    itb_encryptor_t *enc;
    seed_kit_triple_t kit;
    itb_mac_t *mac;
    uint8_t *payload;
    size_t   payload_len;
    uint8_t *transcript;
...
static void ctx_free_all(void) {
    for (size_t i = 0; i < g_ctx_registry_len; i++) {
        ...
        if (c->kit.noise != NULL) itb_seed_free(c->kit.noise);
        if (c->kit.d1 != NULL) itb_seed_free(c->kit.d1);
        if (c->kit.d2 != NULL) itb_seed_free(c->kit.d2);
        if (c->kit.d3 != NULL) itb_seed_free(c->kit.d3);
        if (c->kit.s1 != NULL) itb_seed_free(c->kit.s1);
        if (c->kit.s2 != NULL) itb_seed_free(c->kit.s2);
        if (c->kit.s3 != NULL) itb_seed_free(c->kit.s3);
...
static void run_easy_encrypt_aead_io(void *vctx, uint64_t iters) {
    ...
    for (uint64_t i = 0; i < iters; i++) {
        ...
        if (s != ITB_OK) {
            fprintf(stderr, "easy stream_encrypt_auth (triple) failed: %s\n", itb_last_error());
            abort();
        }
        mem_writer_reset(&w);
    }
...
static void run_easy_decrypt_aead_io(void *vctx, uint64_t iters) {
    ...
    for (uint64_t i = 0; i < iters; i++) {
        ...
        itb_status_t s = itb_encryptor_stream_decrypt_auth(
            c->enc, mem_read_fn, &r, mem_write_fn, &w, STREAM_CHUNK_BYTES);
        if (s != ITB_OK) {
            fprintf(stderr, "easy stream_decrypt_auth (triple) failed: %s\n", itb_last_error());
            abort();
        }
        mem_writer_reset(&w);
    }
...
static void run_easy_encrypt_userloop(void *vctx, uint64_t iters) {
    ...
    for (uint64_t i = 0; i < iters; i++) {
        ...
        while (off < c->payload_len) {
            ...
            if (s != ITB_OK) {
                fprintf(stderr, "easy encrypt (triple) failed: %s\n", itb_last_error());
                abort();
            }
    ...
    for (uint64_t i = 0; i < iters; i++) {
        ...
        while (off + 4 <= c->transcript_len) {
            ...
            if (off + ct_len > c->transcript_len) {
                fprintf(stderr, "easy decrypt userloop (triple): truncated transcript\n");
                abort();
            }
            uint8_t *pt = NULL;
            size_t pt_len = 0;
            ...
            if (s != ITB_OK) {
                fprintf(stderr, "easy decrypt (triple) failed: %s\n", itb_last_error());
                abort();
            }
            if (mem_write_grow(&w, w.len + pt_len) != 0) {
                fprintf(stderr, "easy decrypt userloop (triple): grow failed\n");
                abort();
            }
...
static void run_lowlevel_encrypt_aead_io(void *vctx, uint64_t iters) {
    ...
    for (uint64_t i = 0; i < iters; i++) {
        mem_reader_t r = { c->payload, c->payload_len, 0 };
        mem_writer_t w; mem_writer_init(&w, STREAM_TOTAL_BYTES + (STREAM_TOTAL_BYTES >> 3));
        itb_status_t s = itb_stream_encrypt_auth_triple(
            c->kit.noise, c->kit.d1, c->kit.d2, c->kit.d3,
            c->kit.s1, c->kit.s2, c->kit.s3, c->mac,
            mem_read_fn, &r, mem_write_fn, &w, STREAM_CHUNK_BYTES);
        if (s != ITB_OK) {
            fprintf(stderr, "low-level stream_encrypt_auth_triple failed: %s\n", itb_last_error());
            abort();
        }
        mem_writer_reset(&w);
    }
...
static void run_lowlevel_decrypt_aead_io(void *vctx, uint64_t iters) {
    ...
    for (uint64_t i = 0; i < iters; i++) {
        mem_reader_t r = { c->transcript, c->transcript_len, 0 };
        mem_writer_t w; mem_writer_init(&w, STREAM_TOTAL_BYTES);
        itb_status_t s = itb_stream_decrypt_auth_triple(
            c->kit.noise, c->kit.d1, c->kit.d2, c->kit.d3,
            c->kit.s1, c->kit.s2, c->kit.s3, c->mac,
            mem_read_fn, &r, mem_write_fn, &w, STREAM_CHUNK_BYTES);
        if (s != ITB_OK) {
            fprintf(stderr, "low-level stream_decrypt_auth_triple failed: %s\n", itb_last_error());
            abort();
        }
        mem_writer_reset(&w);
    }
...
static void run_lowlevel_encrypt_userloop(void *vctx, uint64_t iters) {
    ...
    for (uint64_t i = 0; i < iters; i++) {
        ...
        while (off < c->payload_len) {
            ...
            uint8_t *ct = NULL;
            size_t ct_len = 0;
            itb_status_t s = itb_encrypt_triple(
                c->kit.noise, c->kit.d1, c->kit.d2, c->kit.d3,
                c->kit.s1, c->kit.s2, c->kit.s3,
                c->payload + off, end - off, &ct, &ct_len);
            if (s != ITB_OK) {
                fprintf(stderr, "low-level encrypt_triple failed: %s\n", itb_last_error());
                abort();
            }
            frame_chunk(&w, ct, ct_len);
            itb_buffer_free(ct);
...
static void run_lowlevel_decrypt_userloop(void *vctx, uint64_t iters) {
    ...
    for (uint64_t i = 0; i < iters; i++) {
        ...
        while (off + 4 <= c->transcript_len) {
            ...
            off += 4;
            if (off + ct_len > c->transcript_len) {
                fprintf(stderr, "low-level decrypt userloop (triple): truncated transcript\n");
                abort();
            }
            uint8_t *pt = NULL;
            size_t pt_len = 0;
            itb_status_t s = itb_decrypt_triple(
                c->kit.noise, c->kit.d1, c->kit.d2, c->kit.d3,
                c->kit.s1, c->kit.s2, c->kit.s3,
                c->transcript + off, ct_len, &pt, &pt_len);
            if (s != ITB_OK) {
                fprintf(stderr, "low-level decrypt_triple failed: %s\n", itb_last_error());
                abort();
            }
            if (mem_write_grow(&w, w.len + pt_len) != 0) {
                fprintf(stderr, "low-level decrypt userloop (triple): grow failed\n");
                abort();
            }
            memcpy(w.buf + w.len, pt, pt_len);
            w.len += pt_len;
...
static void prebuild_easy_aead_transcript(case_ctx_t *c) {
    ...
    itb_status_t s = itb_encryptor_stream_encrypt_auth(
        c->enc, mem_read_fn, &r, mem_write_fn, &w, STREAM_CHUNK_BYTES);
    if (s != ITB_OK) {
        fprintf(stderr, "prebuild easy AEAD transcript (triple) failed: %s\n", itb_last_error());
        abort();
    }
    c->transcript = w.buf;
...
static void prebuild_lowlevel_aead_transcript(case_ctx_t *c) {
    ...
    itb_status_t s = itb_stream_encrypt_auth_triple(
        ...
        mem_read_fn, &r, mem_write_fn, &w, STREAM_CHUNK_BYTES);
    if (s != ITB_OK) {
        fprintf(stderr, "prebuild low-level AEAD transcript (triple) failed: %s\n", itb_last_error());
        abort();
    }
    c->transcript = w.buf;
    c->transcript_len = w.len;
...
static void prebuild_easy_userloop_transcript(case_ctx_t *c) {
    ...
    while (off < c->payload_len) {
        ...
        itb_status_t s = itb_encryptor_encrypt(c->enc, c->payload + off, end - off,
                                                &ct, &ct_len);
        if (s != ITB_OK) {
            fprintf(stderr, "prebuild easy UserLoop chunk (triple) failed: %s\n", itb_last_error());
            abort();
        }
        frame_chunk(&w, ct, ct_len);
        itb_buffer_free(ct);
...
static void prebuild_lowlevel_userloop_transcript(case_ctx_t *c) {
    ...
    while (off < c->payload_len) {
        ...
        itb_status_t s = itb_encrypt_triple(
            ...
            c->payload + off, end - off, &ct, &ct_len);
        if (s != ITB_OK) {
            fprintf(stderr, "prebuild low-level UserLoop chunk (triple) failed: %s\n", itb_last_error());
            abort();
        }
        frame_chunk(&w, ct, ct_len);
        itb_buffer_free(ct);
...
static bench_case_t make_easy_encrypt_aead_io(char *name) {
    ...
    c->enc = build_stream_encryptor_triple();
...
static bench_case_t make_lowlevel_encrypt_aead_io(char *name) {
    case_ctx_t *c = ctx_new();
    build_stream_seeds_triple(&c->kit);
...
#define NAME_PREFIX  "bench_triple_stream_areion512_1024bit_64mb"
...
int main(void) {
    ...
    printf("# triple_stream payload_bytes=%zu chunk_bytes=%zu primitive=%s "
