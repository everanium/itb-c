/*
 * test_chacha20.c — ChaCha20-focused C binding coverage.
 *
 * Mirrors bindings/rust/tests/test_chacha20.rs one-to-one. ChaCha20
 * ships at a single width (-256) and is ITB's only stream-cipher-based hash. Each Rust `#[test] fn` becomes a
...
static const struct { const char *name; int width; } CHACHA20_HASHES[] = {
    {"chacha20", 256},
};
#define CHACHA20_HASHES_COUNT (sizeof(CHACHA20_HASHES) / sizeof(CHACHA20_HASHES[0]))
...
START_TEST(test_chacha20_roundtrip_across_nonce_sizes)
{
    ...
    for (size_t i = 0; i < NONCE_SIZES_COUNT; i++) {
        for (size_t j = 0; j < CHACHA20_HASHES_COUNT; j++) {
            const char *hash_name = CHACHA20_HASHES[j].name;
...
START_TEST(test_chacha20_triple_roundtrip_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_chacha20_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_chacha20_triple_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_chacha20_persistence_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    ...
    for (size_t hi = 0; hi < CHACHA20_HASHES_COUNT; hi++) {
        const char *hash_name = CHACHA20_HASHES[hi].name;
        int width = CHACHA20_HASHES[hi].width;
...
START_TEST(test_chacha20_roundtrip_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_chacha20_invariants)
{
    ...
    for (size_t hi = 0; hi < CHACHA20_HASHES_COUNT; hi++) {
        const char *hash_name = CHACHA20_HASHES[hi].name;
        int expected_width = CHACHA20_HASHES[hi].width;
        itb_seed_t *ns = NULL, *ds = NULL, *ss = NULL;
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ns), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ds), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ss), ITB_OK);
...
int main(void)
{
    Suite *s = suite_create("chacha20");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_chacha20_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_chacha20_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_chacha20_auth_across_nonce_sizes);
    tcase_add_test(tc, test_chacha20_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_chacha20_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_chacha20_roundtrip_sizes);
    tcase_add_test(tc, test_chacha20_invariants);
