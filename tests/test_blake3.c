/*
 * test_blake3.c — BLAKE3-focused C binding coverage.
 *
 * Mirrors bindings/rust/tests/test_blake3.rs one-to-one. BLAKE3
...
START_TEST(test_blake3_roundtrip_across_nonce_sizes)
{
...
START_TEST(test_blake3_triple_roundtrip_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_blake3_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_blake3_triple_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_blake3_persistence_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_blake3_roundtrip_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_blake3_invariants)
{
    ...
    for (size_t hi = 0; hi < BLAKE3_HASHES_COUNT; hi++) {
        const char *hash_name = BLAKE3_HASHES[hi].name;
        int expected_width = BLAKE3_HASHES[hi].width;
        itb_seed_t *ns = NULL, *ds = NULL, *ss = NULL;
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ns), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ds), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ss), ITB_OK);
...
int main(void)
{
    Suite *s = suite_create("blake3");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_blake3_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_blake3_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_blake3_auth_across_nonce_sizes);
    tcase_add_test(tc, test_blake3_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_blake3_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_blake3_roundtrip_sizes);
    tcase_add_test(tc, test_blake3_invariants);
