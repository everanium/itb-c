/*
 * test_easy_areion.c — Areion-focused Encryptor (Easy Mode) coverage.
 *
 * Mirrors bindings/rust/tests/test_easy_areion.rs one-to-one. Iterates
...
START_TEST(test_easy_areion_roundtrip_across_nonce_sizes)
{
    size_t pt_len = 1024;
...
START_TEST(test_easy_areion_triple_roundtrip_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_areion_auth_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_areion_triple_auth_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_areion_persistence_across_nonce_sizes)
{
...
START_TEST(test_easy_areion_roundtrip_sizes)
{
...
int main(void)
{
    Suite *s = suite_create("easy_areion");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_easy_areion_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_easy_areion_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_easy_areion_auth_across_nonce_sizes);
    tcase_add_test(tc, test_easy_areion_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_easy_areion_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_easy_areion_roundtrip_sizes);
