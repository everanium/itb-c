/*
 * test_aescmac.c — AES-CMAC-focused C binding coverage.
 *
 * W128 counterpart to test_chacha20.c / test_blake3.c: the same
 ...
 * plaintext-size sweep) applied to aescmac. AES-CMAC ships at a single
 * width (128 — the AES block size) so this file iterates the single
 * primitive across the same axes test_chacha20.c covers.
 *
 * Mirrors bindings/rust/tests/test_aescmac.rs one-to-one. Each Rust
 * `#[test] fn` becomes a single START_TEST / END_TEST block here; the
 * cargo serial-lock guard is implicit because each test_*.c file is
 * built as its own binary, giving per-process isolation against the
 * libitb global nonce-bits state.
 */
 ...
 * size). The width feeds into the persistence test's key_bits filter. */
static const struct { const char *name; int width; } AESCMAC_HASHES[] = {
    {"aescmac", 128},
};
#define AESCMAC_HASHES_COUNT (sizeof(AESCMAC_HASHES) / sizeof(AESCMAC_HASHES[0]))
...
/* Generates a pseudo-random byte buffer of length `n`. Mirrors the
 * Rust token_bytes helper — a SplitMix-style LCG seeded from a static
 * counter so successive calls return different bytes within one run
 * without a runtime random dependency. */
...
START_TEST(test_aescmac_roundtrip_across_nonce_sizes)
{
    ...
    for (size_t i = 0; i < NONCE_SIZES_COUNT; i++) {
        for (size_t j = 0; j < AESCMAC_HASHES_COUNT; j++) {
            const char *hash_name = AESCMAC_HASHES[j].name;
...
START_TEST(test_aescmac_triple_roundtrip_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_aescmac_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_aescmac_triple_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_aescmac_persistence_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    ...
    for (size_t hi = 0; hi < AESCMAC_HASHES_COUNT; hi++) {
        const char *hash_name = AESCMAC_HASHES[hi].name;
        int width = AESCMAC_HASHES[hi].width;
        ...
        for (size_t ki = 0; ki < sizeof(candidate_kb) / sizeof(candidate_kb[0]); ki++) {
            ...
            for (size_t i = 0; i < NONCE_SIZES_COUNT; i++) {
                ...
                /* Extract components + hash key from each seed. */
...
START_TEST(test_aescmac_roundtrip_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_aescmac_invariants)
{
    /* Width invariant + hash-key length contract + NULL no-op
     * contracts. The libitb primitive rejects zero-length plaintext at
     * the FFI layer (returns ITB_ENCRYPT_FAILED, not ITB_OK), so the
     * empty-roundtrip path is exercised as a defined-rejection rather
     * than a roundtrip. */
    itb_seed_t *ns = NULL, *ds = NULL, *ss = NULL;
    ck_assert_int_eq(itb_seed_new("aescmac", 1024, &ns), ITB_OK);
    ck_assert_int_eq(itb_seed_new("aescmac", 1024, &ds), ITB_OK);
    ck_assert_int_eq(itb_seed_new("aescmac", 1024, &ss), ITB_OK);

    int width = 0;
    ck_assert_int_eq(itb_seed_width(ns, &width), ITB_OK);
    ck_assert_int_eq(width, 128);

    /* Confirm itb_seed_hash_key returns exactly 16 bytes for aescmac. */
    uint8_t key[64];
    size_t kl = 0;
    ck_assert_int_eq(itb_seed_hash_key(ns, key, sizeof(key), &kl), ITB_OK);
    ck_assert_uint_eq(kl, 16);

    /* Defined rejection of zero-length plaintext — buffer is NULL +
     * length 0 on rejection. */
    ...
    /* NULL no-op contracts — must not crash. */
...
int main(void)
{
    Suite *s = suite_create("aescmac");
    TCase *tc = tcase_create("core");
    /* The plaintext-size sweep encrypts a 1 MiB chunk; bump the per-test
     * timeout from check's 4-second default so slow CI hardware does not
     * spuriously fail the run. */
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_aescmac_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_aescmac_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_aescmac_auth_across_nonce_sizes);
    tcase_add_test(tc, test_aescmac_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_aescmac_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_aescmac_roundtrip_sizes);
    tcase_add_test(tc, test_aescmac_invariants);
    suite_add_tcase(s, tc);

    SRunner *sr = srunner_create(s);
    /* No-fork mode keeps the per-test timeout under our control (the
     * fork-mode timer is forced from elsewhere on this platform) and
     * gives every test access to the libitb global state without
     * cross-test fork copies. */
