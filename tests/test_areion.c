/*
 * test_areion.c — Areion-SoEM-focused C binding coverage.
 *
 * Symmetric counterpart to test_blake2b.c: the same coverage shape
 * (nonce-size sweep, single + triple roundtrip, single + triple auth
 * with tamper rejection, persistence sweep, plaintext-size sweep)
 * applied to areion256 / areion512.
 *
 ...
 * into the persistence test's key_bits filter. */
static const struct { const char *name; int width; } AREION_HASHES[] = {
    {"areion256", 256},
    {"areion512", 512},
};
#define AREION_HASHES_COUNT (sizeof(AREION_HASHES) / sizeof(AREION_HASHES[0]))
...
START_TEST(test_areion_roundtrip_across_nonce_sizes)
{
    ...
    for (size_t i = 0; i < NONCE_SIZES_COUNT; i++) {
        for (size_t j = 0; j < AREION_HASHES_COUNT; j++) {
            const char *hash_name = AREION_HASHES[j].name;
...
START_TEST(test_areion_triple_roundtrip_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_areion_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_areion_triple_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_areion_persistence_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    ...
    for (size_t hi = 0; hi < AREION_HASHES_COUNT; hi++) {
        const char *hash_name = AREION_HASHES[hi].name;
        int width = AREION_HASHES[hi].width;
...
START_TEST(test_areion_roundtrip_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_areion_invariants)
{
    ...
    for (size_t hi = 0; hi < AREION_HASHES_COUNT; hi++) {
        const char *hash_name = AREION_HASHES[hi].name;
        int expected_width = AREION_HASHES[hi].width;
        itb_seed_t *ns = NULL, *ds = NULL, *ss = NULL;
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ns), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ds), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ss), ITB_OK);
...
int main(void)
{
    Suite *s = suite_create("areion");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_areion_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_areion_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_areion_auth_across_nonce_sizes);
    tcase_add_test(tc, test_areion_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_areion_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_areion_roundtrip_sizes);
    tcase_add_test(tc, test_areion_invariants);
