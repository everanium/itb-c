/*
 * test_easy_chacha20.c — ChaCha20-focused Encryptor (Easy Mode) coverage.
 *
 * Mirrors bindings/rust/tests/test_easy_chacha20.rs one-to-one.
 * ChaCha20 ships at a single width (-256). Per-binary fork() isolation
 * gives this test its own libitb global state, so no in-process serial
 * lock is required.
 *
...
START_TEST(test_easy_chacha20_roundtrip_across_nonce_sizes)
{
    size_t pt_len = 1024;
...
START_TEST(test_easy_chacha20_triple_roundtrip_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_chacha20_auth_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_chacha20_triple_auth_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_chacha20_persistence_across_nonce_sizes)
{
...
START_TEST(test_easy_chacha20_roundtrip_sizes)
{
...
int main(void)
{
    Suite *s = suite_create("easy_chacha20");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_easy_chacha20_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_easy_chacha20_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_easy_chacha20_auth_across_nonce_sizes);
    tcase_add_test(tc, test_easy_chacha20_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_easy_chacha20_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_easy_chacha20_roundtrip_sizes);
