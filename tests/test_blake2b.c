/*
 * test_blake2b.c — BLAKE2b-focused C binding coverage.
 *
 * Symmetric counterpart to test_areion.c: the same coverage shape
 * (nonce-size sweep, single + triple roundtrip, single + triple auth
 * with tamper rejection, persistence sweep, plaintext-size sweep)
 * applied to blake2b256 / blake2b512.
 *
 * Mirrors bindings/rust/tests/test_blake2b.rs one-to-one. Each Rust
...
/* (hash, ITB_seed_width) — both BLAKE2b widths. The width feeds
 ...
 * contract that BLAKE2b-256 carries a 32-byte fixed key and
 * BLAKE2b-512 carries a 64-byte fixed key. */
...
START_TEST(test_blake2b_roundtrip_across_nonce_sizes)
{
...
START_TEST(test_blake2b_triple_roundtrip_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_blake2b_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_blake2b_triple_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_blake2b_persistence_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_blake2b_roundtrip_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_blake2b_invariants)
{
    ...
    for (size_t hi = 0; hi < BLAKE2B_HASHES_COUNT; hi++) {
        const char *hash_name = BLAKE2B_HASHES[hi].name;
        int expected_width = BLAKE2B_HASHES[hi].width;
        itb_seed_t *ns = NULL, *ds = NULL, *ss = NULL;
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ns), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ds), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ss), ITB_OK);
...
int main(void)
{
    Suite *s = suite_create("blake2b");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_blake2b_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_blake2b_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_blake2b_auth_across_nonce_sizes);
    tcase_add_test(tc, test_blake2b_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_blake2b_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_blake2b_roundtrip_sizes);
    tcase_add_test(tc, test_blake2b_invariants);
