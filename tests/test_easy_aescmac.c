/*
 * test_easy_aescmac.c — AES-CMAC-focused Encryptor (Easy Mode) coverage.
 *
 * Mirrors bindings/rust/tests/test_easy_aescmac.rs one-to-one.
 * AES-CMAC ships only at -128 (the AES block size). Each Rust
 * `#[test] fn` becomes a single START_TEST / END_TEST block here;
...
START_TEST(test_easy_aescmac_roundtrip_across_nonce_sizes)
{
    size_t pt_len = 1024;
...
START_TEST(test_easy_aescmac_triple_roundtrip_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_aescmac_auth_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_aescmac_triple_auth_across_nonce_sizes)
{
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_easy_aescmac_persistence_across_nonce_sizes)
{
...
START_TEST(test_easy_aescmac_roundtrip_sizes)
{
...
int main(void)
{
    Suite *s = suite_create("easy_aescmac");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_easy_aescmac_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_easy_aescmac_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_easy_aescmac_auth_across_nonce_sizes);
    tcase_add_test(tc, test_easy_aescmac_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_easy_aescmac_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_easy_aescmac_roundtrip_sizes);
