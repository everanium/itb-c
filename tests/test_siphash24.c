/*
 * test_siphash24.c — SipHash-2-4-focused C binding coverage.
 *
 * Mirrors bindings/rust/tests/test_siphash24.rs one-to-one. SipHash-2-4
 * ships at a single width (-128) and has no internal fixed key — the keying material is the per-call seed components — so expected_key_len returns 0 and the persistence path stores empty bytes for the hash key. Each Rust `#[test] fn` becomes a
...
START_TEST(test_siphash24_roundtrip_across_nonce_sizes)
{
...
START_TEST(test_siphash24_triple_roundtrip_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_siphash24_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_siphash24_triple_auth_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
    size_t pt_len = 1024;
    uint8_t *plaintext = token_bytes(pt_len);
...
START_TEST(test_siphash24_persistence_across_nonce_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_siphash24_roundtrip_sizes)
{
    int orig = itb_get_nonce_bits();
...
START_TEST(test_siphash24_invariants)
{
    ...
    for (size_t hi = 0; hi < SIPHASH_HASHES_COUNT; hi++) {
        const char *hash_name = SIPHASH_HASHES[hi].name;
        int expected_width = SIPHASH_HASHES[hi].width;
        itb_seed_t *ns = NULL, *ds = NULL, *ss = NULL;
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ns), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ds), ITB_OK);
        ck_assert_int_eq(itb_seed_new(hash_name, 1024, &ss), ITB_OK);
...
int main(void)
{
    Suite *s = suite_create("siphash24");
    TCase *tc = tcase_create("core");
    tcase_set_timeout(tc, 60);
    tcase_add_test(tc, test_siphash24_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_siphash24_triple_roundtrip_across_nonce_sizes);
    tcase_add_test(tc, test_siphash24_auth_across_nonce_sizes);
    tcase_add_test(tc, test_siphash24_triple_auth_across_nonce_sizes);
    tcase_add_test(tc, test_siphash24_persistence_across_nonce_sizes);
    tcase_add_test(tc, test_siphash24_roundtrip_sizes);
    tcase_add_test(tc, test_siphash24_invariants);
